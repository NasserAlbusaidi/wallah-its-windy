...
| `realism-proxy.ts` | THE single import surface for the R2a BT-proxy measurement layer: owns the debris recurrence and re-exports `realism-glsl` + `realism-field`. A measurement instrument, not a renderer — no render path or `main.ts` imports it. | `computeDebrisState`, `DebrisState`, plus the re-exported `realism-glsl`/`realism-field` surface |
| `realism-glsl.ts` | CPU mirrors of the GLSL scalar semantics the shader depends on (`fract` correct for negatives, CLOCKWISE `rotate2`, `hash21`) plus the shared cloud-noise sampler. The leaf module: imports no sibling, so the harness graph stays acyclic. | `glslFract`, `glslRotate2`, `glslHash21`, `RealismNoise`, `cellUv` |
| `realism-cloud-sample.ts` | Statement-by-statement CPU transcription of the env shader's `sampleCloud()` with the cloud-motion splices inlined, kept apart from the rasterizer so it diffs line-for-line against the GLSL. Documented skips: relief shading, palette compositing. | `sampleCloudProxy`, `CloudUniforms`, `CloudCellInputs`, `ProxyCloudField` |
| `realism-field.ts` | The measurement-field rasterizer: quantized env-plane sampling (quantize BEFORE filtering — the order is load-bearing), the renderer's plane/blend mirror, and a walk of the `sampleCloud()` twin over a fixed 192² grid. | `buildRealismField`, `RealismField`, `quantizedLayerSampler`, `midlevelRhUniform`, `REALISM_GRID_N` |
| `realism-metrics.ts` | The RGR shortlist metrics over the BT-proxy field: environmental cloud fraction, eye contrast, cold-top area/centroid, shear-relative band edge energy. Returns null, never a placeholder, when a defining population or direction is unavailable. | `metricsForField`, `RealismFrameMetrics`, `RealismColdTop`, `REALISM_COLD_TOP_C`, `REALISM_MIN_SHEAR_DIR_MS` |
